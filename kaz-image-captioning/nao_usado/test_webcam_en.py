...
USANDO VOCABULÁRIO EM INGLÊS DIRETO (vocab_en.pickle)
...
# Configurações do modelo - USANDO VOCABULÁRIO EM INGLÊS
load_path = 'checkpoints/kaz_model.pth'
dict_path = 'vocabulary/vocab_en.pickle'  # ← INGLÊS!
img_size = 384

print("🔄 Carregando dicionário em INGLÊS...")
with open(dict_path, 'rb') as f:
    coco_tokens = pickle.load(f)
print("✅ Dicionário em inglês carregado!")
...
def generate_caption(img):
    """Gera legenda para uma imagem em INGLÊS"""
    print("\n🤖 Gerando legenda em inglês...")
    start = time()
    ...
    stop = time()
    print(f'📝 Descrição (English): {caption}')
...
def main():
    ...
    print("  👉 Legendas em INGLÊS (usando vocab_en.pickle)")
    ...
    try:
        while True:
            ...
            cv2.putText(display_frame, "Legendas em INGLES (vocab_en.pickle)", 
                       (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            cv2.putText(display_frame, "Pressione 'ESC' ou 'q' para sair", 
                       (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            cv2.imshow("Webcam - Image Captioning (ENGLISH)", display_frame)
            ...
            # 'c' ou ESPAÇO para capturar
                ...
                capture_filename = f'captured_english_{capture_count}.jpg'
                cv2.imwrite(capture_filename, frame)
                print(f"💾 Imagem salva: {capture_filename}")
                
                # Gerar legenda em inglês
                ...
                cv2.putText(result_frame, "English Caption:", 
                          (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                
                # Dividir caption em múltiplas linhas se necessário
                max_chars = 50
                caption_lines = [caption[i:i+max_chars] for i in range(0, len(caption), max_chars)]
                for i, line in enumerate(caption_lines[:2]):  # Máximo 2 linhas
                    cv2.putText(result_frame, line, 
                              (10, y_offset + 35 + i*25), cv2.FONT_HERSHEY_SIMPLEX, 
                              0.6, (255, 255, 255), 2)
                
                cv2.imshow(f"Result #{capture_count} - English", result_frame)
                ...
                # Salvar legenda em arquivo texto
                with open(f'caption_english_{capture_count}.txt', 'w', encoding='utf-8') as f:
                    f.write(f"English Caption: {caption}\n")
                print(f"📝 Legenda salva: caption_english_{capture_count}.txt")
