...
COM TRADUÇÃO AUTOMÁTICA PARA INGLÊS
Pressione 'c' para capturar uma imagem e gerar a legenda em INGLÊS
...
# Dicionário de tradução Kazakh -> Inglês (palavras mais comuns)
KAZAKH_TO_ENGLISH = {
    # Pessoas
    'адам': 'person',
    'ер адам': 'man',
...
def translate_kazakh_to_english(kazakh_text):
    """
    Traduz texto de Kazakh para Inglês palavra por palavra
    Mantém palavras desconhecidas em Kazakh
    """
    words = kazakh_text.split()
    translated_words = []
    ...
    for word in words:
        ...
        # Buscar tradução (case-insensitive)
        translated = KAZAKH_TO_ENGLISH.get(word_clean.lower(), word_clean)
        
        # Manter capitalização se a palavra original estava capitalizada
...
def generate_caption(img):
    ...
    caption_english = translate_kazakh_to_english(caption_kazakh)
...
def main():
    ...
    print("  👉 Legendas serão mostradas em KAZAKH e INGLÊS")
    ...
    try:
        while True:
            ...
            cv2.putText(display_frame, "Legendas em Kazakh + English", 
                       (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            cv2.putText(display_frame, "Pressione 'ESC' ou 'q' para sair", 
                       (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            cv2.imshow("Webcam - Image Captioning (KZ + EN)", display_frame)
            ...
            # 'c' ou ESPAÇO para capturar
                ...
                capture_filename = f'captured_frame_{capture_count}.jpg'
                cv2.imwrite(capture_filename, frame)
                print(f"💾 Imagem salva: {capture_filename}")
                
                # Gerar legenda em ambos idiomas
                ...
                # Kazakh
                cv2.putText(result_frame, "Kazakh:", 
                          (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                cv2.putText(result_frame, caption_kz[:60], 
                          (120, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                
                # English
                cv2.putText(result_frame, "English:", 
                          (10, y_offset + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                cv2.putText(result_frame, caption_en[:60], 
                          (120, y_offset + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                
                cv2.imshow(f"Resultado #{capture_count}", result_frame)
                
                # Salvar resultado
                result_filename = f'result_english_{capture_count}.jpg'
                cv2.imwrite(result_filename, result_frame)
                print(f"💾 Resultado salvo: {result_filename}")
                
                # Salvar texto em arquivo
                with open(f'caption_{capture_count}.txt', 'w', encoding='utf-8') as f:
                    f.write(f"Kazakh: {caption_kz}\n")
                    f.write(f"English: {caption_en}\n")
                print(f"💾 Legendas salvas: caption_{capture_count}.txt")
