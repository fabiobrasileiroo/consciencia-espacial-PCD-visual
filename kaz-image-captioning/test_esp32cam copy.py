...
if model_available:
    try:
        # Tentar carregar o modelo para verificar se está válido
        checkpoint = torch.load(load_path, map_location='cpu')
        print("✅ Modelo encontrado e válido!")
    except Exception as e:
        print(f"⚠️  Modelo encontrado mas corrompido: {e}")
        print("📝 Continuando sem modelo (apenas captura de imagem)")
...
# Configurações do modelo (apenas se disponível)
if model_available:
    ...
    print("📝 Modo sem modelo ativado - apenas captura de imagens")
...
def generate_caption(img, translate=True):
    ...
    if not model_available:
        print("\n🤖 Modo sem modelo - apenas salvando imagem")
        print("📝 Para usar geração de legendas, baixe o modelo kaz_model.pth")
        return "Imagem capturada (sem legenda - modelo não disponível)", "Imagem capturada (sem legenda - modelo não disponível)"
        ...
        while True:
            ...
            # 'c' ou ESPAÇO para capturar
                ...
                if model_available:
                    ...
                    # Modo sem modelo - apenas mostrar mensagem
                    cv2.rectangle(overlay, (0, result_frame.shape[0] - 60), 
                                (result_frame.shape[1], result_frame.shape[0]), 
                                (0, 0, 0), -1)
                    cv2.addWeighted(overlay, 0.7, result_frame, 0.3, 0, result_frame)
                    
                    cv2.putText(result_frame, "Modo sem modelo - apenas captura", 
                              (10, result_frame.shape[0] - 30), cv2.FONT_HERSHEY_SIMPLEX, 
                              0.7, (255, 255, 255), 2)
                    cv2.putText(result_frame, "Baixe kaz_model.pth para legendas", 
                ...
                with open(caption_filename, 'w', encoding='utf-8') as f:
                    if model_available:
                        ...
                        f.write("Modo sem modelo ativado\n")
                        f.write("Para gerar legendas, baixe o modelo kaz_model.pth\n")
                        f.write("Link: https://drive.google.com/drive/folders/16PDZvoNs3P-O9Vr3zEb6bb-aaSDOiSY0\n")
