...
# Labels COCO em inglês e português
COCO_LABELS = [
...
# API para envio das detecções
API_URL = "http://localhost:8000/detections"  
SEND_INTERVAL = 2.0 
detection_queue = deque(maxlen=100)  
last_send_time = 0

# =========================
# FUNÇÕES DE ENVIO E UTILS
# =========================
...
def iou_xyxy(a, b):
    """Calcula IoU entre dois boxes no formato xyxy"""
    xa1, ya1, xa2, ya2 = a
    xb1, yb1, xb2, yb2 = b
...
# =========================
# TRACKER DE DETECÇÕES
# =========================

class DetectionTracker:
    """Rastreamento simples de detecções para confirmar objetos persistentes"""
    def __init__(self):
        self.tracks = []

    def update(self, candidates):
        # Atualiza idade dos rastros existentes
...
def xywh2xyxy(x):
    """Converte box de xywh para xyxy"""
    y = np.copy(x)
    y[..., 0] = x[..., 0] - x[..., 2] / 2.0
    y[..., 1] = x[..., 1] - x[..., 3] / 2.0
    y[..., 2] = x[..., 0] + x[..., 2] / 2.0
...
def do_nms(boxes, scores, iou_threshold=IOU_THRESHOLD, max_output=50):
    """Non-Maximum Suppression usando TensorFlow"""
    if len(boxes) == 0:
...
def process_stream(url=URL):
    """
    Função que conecta ao stream, processa os frames, faz detecção,
    exibe na tela e envia detecções para API periodicamente.
    """
                    ...
                    if preds.ndim == 2 and preds.shape[1] >= 6:
                        ...
                        if preds.shape[0] == 0:
                            ...
                            for t in confirmed:
                                ...
                                label_name_pt = COCO_LABELS_PT.get(label_name, label_name)
...
# =========================
# CLASSE PARA LER O STREAM E DISTRIBUIR FRAMES
# =========================
...
# =========================
# INICIALIZAÇÃO DA FILA E STREAM
# =========================

frame_queue = queue.Queue(maxsize=10)  
stream_reader = StreamReader(URL, frame_queue)
stream_reader.start() 

# =========================
# EXEMPLO DE CONSUMIDOR SIMPLES (NÃO USADO)
# =========================
...
frame_results = {}  # Dicionário para armazenar resultados por frame_id
frame_lock = threading.Lock()  # Lock para sincronização entre threads

def process_my_model():
    while True:
        frame = frame_queue.get()
        frame_id = int(time.time() * 1000)
        payload_my = {"model": "meu_modelo", "result": "resultado_do_meu_modelo"}
        with frame_lock:
            if frame_id not in frame_results:
                frame_results[frame_id] = {}
            frame_results[frame_id]['my'] = payload_my
            # Se o outro modelo já terminou, envia o payload combinado
            if 'other' in frame_results[frame_id]:
                send_both_payloads(frame_results[frame_id])
                del frame_results[frame_id]

def process_other_model():
    """
    Thread que consome frames da fila e processa com OUTRO modelo.
    Quando terminar, armazena o resultado e verifica se o seu modelo já terminou.
    """
    while True:
        frame = frame_queue.get()
        frame_id = int(time.time() * 1000)
        result = run_kaz_model(frame)
        payload_other = {"model": "kaz_model", "result": result}
        with frame_lock:
            if frame_id not in frame_results:
                frame_results[frame_id] = {}
            frame_results[frame_id]['other'] = payload_other
            if 'my' in frame_results[frame_id]:
                send_both_payloads(frame_results[frame_id])
                del frame_results[frame_id]

def send_both_payloads(results):
    """
    Função chamada quando ambos os modelos terminaram de processar o mesmo frame.
    Aqui você pode montar e enviar o payload combinado para a API.
    """
    print("Enviando payload combinado:", results)
    # Exemplo:
    # requests.post(API_URL, json=results)

# =========================
# INICIALIZAÇÃO DAS THREADS DE PROCESSAMENTO
# =========================

if __name__ == "__main__":
    KAZ_MODEL_PATH = r'kaz_model.pth'
    kaz_model = torch.load(KAZ_MODEL_PATH, map_location='cpu')
    kaz_model.eval()

    def run_kaz_model(frame):
        # Pré-processamento: converte BGR para RGB, redimensiona, normaliza, etc.
        img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (224, 224))  # ajuste para o tamanho do seu modelo
        img = img.astype(np.float32) / 255.0
        img = torch.from_numpy(img).permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
        with torch.no_grad():
            output = kaz_model(img)
        pred = torch.argmax(output, dim=1).item()
        prob = torch.softmax(output, dim=1)[0, pred].item()
        return {"class": int(pred), "confidence": float(prob)}

    t1 = threading.Thread(target=process_my_model, daemon=True)
    t2 = threading.Thread(target=process_other_model, daemon=True)
    t1.start()
    t2.start()
    while True:
        time.sleep(1)
