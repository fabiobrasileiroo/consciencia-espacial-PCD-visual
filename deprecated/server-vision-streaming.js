...
app.get('/', (req, res) => {
  res.send(`
    ...
    <html>
      ...
      <body>
        <div class="card">
          <h1>🎯 Vision API - Sistema ESP32</h1>
          <p>Servidor de Visão Computacional para PCD Visual</p>
          <hr>
          <h3>🔗 Links Úteis:</h3>
          <a href="/monitor">📊 Monitor de Eventos (SSE)</a>
...
function handleESP32Message(message) {
  ...
  switch (message.type) {
    ...
    case 'sensor_update':
      // Atualização em tempo real do sensor de distância
...
function handleSensorUpdate(message) {
  ...
  // Broadcast completo via SSE para clientes web/mobile
  ...
  // Broadcast para clientes WebSocket (app mobile)
  broadcastToClients({
    type: 'sensor-update',
    data: {
      distance,
  ...
  // Se for alerta crítico, adicionar ao sistema
